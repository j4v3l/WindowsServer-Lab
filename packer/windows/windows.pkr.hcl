...
variable "windows_setup_key" {
  type      = string
  sensitive = true
  validation {
    condition     = can(regex("^[A-Za-z0-9]{5}(-[A-Za-z0-9]{5}){4}$", var.windows_setup_key))
    error_message = "The Windows setup key must use the five-by-five product-key format."
  }
}
variable "cloudbase_msi_url" { type = string }
variable "cloudbase_msi_sha256" { type = string }
variable "osconfig_nupkg_url" { type = string }
variable "osconfig_nupkg_sha256" { type = string }
variable "osconfig_version" { type = string }
...
variable "os_type" {
  type = string
  validation {
    condition     = contains(["server-2025", "windows-11"], var.os_type)
    error_message = "The os_type value must be server-2025 or windows-11."
  }
}
...
locals {
  ...
  virtio_driver_path = var.os_type == "server-2025" ? "2k25" : "w11"
}
...
source "proxmox-iso" "windows" {
  ...
  template_description     = "WindowsServerLab v3 ${var.os_type}; WSLAB-BUILD-RUN=${var.build_run_id}; Cloudbase-Init and QEMU Guest Agent; certification required"
