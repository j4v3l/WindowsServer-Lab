...
- links only to the selected demo's base OU, which contains lab servers, workstations, and users.
...
3. Copy the repository or release bundle to the machine. The script resolves the module and demo definition from either the release layout or `C:\ProgramData\WindowsServerLab`.
4. Connect the machine to a site network that can reach the selected lab DNS servers and domain controllers.
...
.\Scripts\Set-LabMachineEnrollment.ps1 `
  -Action Enroll -Demo asgard -DeviceType Workstation `
...
```powershell
.\Scripts\Set-LabMachineEnrollment.ps1 -Action Status -Demo asgard
.\Scripts\Set-LabMachineEnrollment.ps1 -Action Enroll -Demo asgard -RefreshPolicy
```
