...
grep -Fq "elseif (\$specializeShellSetup)" "$REPO_ROOT/packer/windows/seal-template.ps1"
grep -Fq "elseif (\$autoLogon)" "$REPO_ROOT/packer/windows/seal-template.ps1"
grep -Fq "AppControl/WindowsServer/2025/DefaultPolicy/Audit" "$REPO_ROOT/Scripts/WindowsServerLab/WindowsServerLab.psm1"
grep -Fq "AppControl/WindowsServer/2025/AppBlockList/Audit" "$REPO_ROOT/Scripts/WindowsServerLab/WindowsServerLab.psm1"
grep -Fq "AllowMicrosoft_WS2025_Audit" "$REPO_ROOT/Scripts/WindowsServerLab/WindowsServerLab.psm1"
grep -Fq "BlockUMCI_Microsoft_WS2025_Audit" "$REPO_ROOT/Scripts/WindowsServerLab/WindowsServerLab.psm1"
! grep -Fq "AppControl\\WS2025" "$REPO_ROOT/Scripts/WindowsServerLab/WindowsServerLab.psm1"
grep -Fq 'Trim([char]0xFEFF)' "$REPO_ROOT/Scripts/Enable-LabEventForwarding.ps1"
grep -Fq 'wecutil.exe ss /c:' "$REPO_ROOT/Scripts/Enable-LabEventForwarding.ps1"
grep -Fq 'The event collector subscription is source-initiated' "$REPO_ROOT/Scripts/Invoke-LabBootstrap.ps1"
grep -Fq "Set-NetFirewallRule -Name \$domainRule.Name -Enabled True -Profile Any" "$REPO_ROOT/Scripts/Enable-LabPowerShellRemoting.ps1"
grep -Fq 'Checksum-verified payload transfer to VM' "$REPO_ROOT/Tools/Proxmox/Configure-LabGuests.sh"
grep -Fq 'failure_mode="${4:-fatal}"' "$REPO_ROOT/Tools/Proxmox/Configure-LabGuests.sh"
grep -Fq 'request_timeout="${5:-$timeout_seconds}"' "$REPO_ROOT/Tools/Proxmox/Configure-LabGuests.sh"
grep -Fq 'baseline_timeout_seconds=3600' "$REPO_ROOT/Tools/Proxmox/Configure-LabGuests.sh"
grep -Fq 'security-baseline-${baseline_role}-${baseline_revision}.complete' "$REPO_ROOT/Tools/Proxmox/Configure-LabGuests.sh"
grep -Fq 'legacy_baseline_revision='"'"'30cb877968757416ed22d5a716d866147ed90c0009328334b1dbeb9817dd8819'"'"'' "$REPO_ROOT/Tools/Proxmox/Configure-LabGuests.sh"
grep -Fq 'Timed out waiting for the security-baseline lock.' "$REPO_ROOT/Tools/Proxmox/Configure-LabGuests.sh"
grep -Fq 'Guest-agent response for VM' "$REPO_ROOT/Tools/Proxmox/Configure-LabGuests.sh"
grep -Fq 'tail -n 12 >&2' "$REPO_ROOT/Tools/Proxmox/Configure-LabGuests.sh"
grep -Fq 'dhcp_converged="false"' "$REPO_ROOT/Tools/Proxmox/Configure-LabGuests.sh"
grep -Fq 'Final DHCP/DNS reconciliation on VM' "$REPO_ROOT/Tools/Proxmox/Configure-LabGuests.sh"
grep -Fq 'policy_converged="false"' "$REPO_ROOT/Tools/Proxmox/Configure-LabGuests.sh"
grep -Fq 'waiting for AD replication' "$REPO_ROOT/Tools/Proxmox/Configure-LabGuests.sh"
grep -Fq 'for ($dnsAttempt = 1; $dnsAttempt -le 20; $dnsAttempt++)' "$REPO_ROOT/Scripts/WindowsServerLab/WindowsServerLab.psm1"
grep -Fq "Remote guest job disappeared before producing a status result" "$REPO_ROOT/Tools/Terraform/Remote-Run.sh"
grep -Fq 'CloudExperienceHostBroker' "$REPO_ROOT/packer/windows/seal-template.ps1"
grep -Fq "WSLAB_WINDOWS_SETUP_KEY=\${var.windows_setup_key}" "$REPO_ROOT/packer/windows/windows.pkr.hcl"
grep -Fq "WSLAB_WINDOWS_BUILD_PASSWORD=\${var.windows_password}" "$REPO_ROOT/packer/windows/windows.pkr.hcl"
...
grep -Fq 'oobeShellCrashFree' "$REPO_ROOT/Tools/Proxmox/Certify-WindowsTemplate.sh"
grep -Fq 'windowsSetupKeyPresent' "$REPO_ROOT/Tools/Proxmox/Certify-WindowsTemplate.sh"
...
grep -Fq 'Scripts/Test-LabGroupPolicyRefresh.ps1|C:\ProgramData\WindowsServerLab\Scripts\Test-LabGroupPolicyRefresh.ps1' "$REPO_ROOT/Tools/Proxmox/Configure-LabGuests.sh"
grep -Fq 'Scripts/Test-LabAccessControl.ps1|C:\ProgramData\WindowsServerLab\Scripts\Test-LabAccessControl.ps1' "$REPO_ROOT/Tools/Proxmox/Configure-LabGuests.sh"
