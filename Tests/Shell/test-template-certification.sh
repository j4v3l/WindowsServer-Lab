...
jq -n \
  ...
  '{
    ...
    template:{id:9000,os:"server-2025",node:"pve01",pveVersion:"pve-manager/9.2.3/test",configSha256:$configHash,automationSha256:$automationHash,buildRunId:"test",sources:{windowsIsoSha256:("a"*64),virtioIsoSha256:("b"*64)},payloads:{cloudbaseInitSha256:("c"*64),qemuGuestAgentSha256:("d"*64)}},
    canaries:[
...
jq -n --arg automationHash "$automation_hash" '{schemaVersion:1,automationSha256:$automationHash}' >"$TEMP_DIR/reports/template-builds/template-server-2025-9000.json"
