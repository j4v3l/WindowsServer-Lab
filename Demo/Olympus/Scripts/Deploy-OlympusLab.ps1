...
New-ADOrganizationalUnit -Name "Servers" -Path "OU=Olympus Systems,DC=olympus,DC=local"
...
foreach ($server in $servers) {
    New-ADComputer -Name $server -Path "OU=Servers,OU=Olympus Systems,DC=olympus,DC=local" -Description "Olympus Systems Server" -Enabled $true
}
...
foreach ($ws in $workstations) {
    New-ADComputer -Name $ws.Name -Path "OU=$($ws.Dept),OU=Departments,OU=Olympus Systems,DC=olympus,DC=local" -Description $ws.Description -Enabled $true
}
